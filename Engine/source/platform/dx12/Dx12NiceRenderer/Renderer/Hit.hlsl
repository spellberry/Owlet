...
float3 RandomInUnitSphere(uint seed1, uint seed2, uint seed3)
{
    ...
    do
    {
        ...
        randomVec =
            2.0 * float3(float(pcg_hash(seed1 + pcg_hash( diff * 7919))) / 0xFFFFFFFF,
...
float3 barycentrics = float3(1.f - attrib.bary.x - attrib.bary.y, attrib.bary.x, attrib.bary.y);
 ...
 float4 norm_color = texture1[tex_index + 1].SampleGrad(s1, tex, 0, 0);
    ...
    { 
       ...
       /* float3 sphere = RandomInUnitSphere(
          ...
          //float3 sphere = RandomInUnitSphere(
          //  float(pcg_hash(i * 7127 +nrand(tex *i* 863) + frame_index * 4937 +
          //                 i * 257 * (barycentrics.x * 4591 + barycentrics.y * 3167 + barycentrics.z * 5801))) ,
          //  float(pcg_hash(i * 4591 + nrand(tex * i * 7919) + frame_index * 7433 +
          //                 i * 3167 * (barycentrics.x * 7127 + barycentrics.y * 4057 + barycentrics.z * 2311))) ,
          //  float(pcg_hash(i * 6841 + nrand(tex * i * 5477) + frame_index * 1993 +
          //                 i * 2141 * (barycentrics.x * 3907 + barycentrics.y * 587 + barycentrics.z * 7127))) );
        
        float3 lightPos = litPos+ sphere*3;

       /* float rand1 =
            float(pcg_hash(nrand(tex) + i * 7127 + frame_index * barycentrics.x * 2311 + frame_index * 3907)) / 0xFFFFFFFF;
      
        float rand2 =
            float(nrand(tex) + pcg_hash(i * 4591 + frame_index * barycentrics.y * 3167 + frame_index * 7919)) / 0xFFFFFFFF;

        float rand3 =
            float(nrand(tex) + pcg_hash(i * 6841 + frame_index * barycentrics.z * 4057 + frame_index * 5801)) / 0xFFFFFFFF;


    
         float3 lightPos = litPos + float3(rand1  - 1, rand2  - 1, rand3 - 1);*/

      /*  if (length(float3(rand1, rand2, rand3)) > 1)
    {
        i--;
        continue;
    }*/
  ...
  /*   if (metal_color.b > 0.9)
      ...
      float3 color_r = float3(0,0,0);
         for (int i = 0; i < sample_count; i++)
         {
             ...
             float3 new_norm = RandomInHemisphere(norm,
                 float(pcg_hash(i * 7127 * nrand(tex * 3167) + frame_index * barycentrics.x * 2311 + frame_index * 3907)),                           
                 float(pcg_hash(i * 6841 + frame_index * barycentrics.z * 4057 + frame_index * 5801) + nrand(tex * 7127)));
  ...
  /*    }
   ...
   /*  hitColor =
         float3(float(pcg_hash(frame_index * 257 + barycentrics.x * 4591 + barycentrics.y * 3167 + barycentrics.z * 5801)) /
                    0xFFFFFFFF, 
                       float(pcg_hash(frame_index * 3167 + 2 * (barycentrics.x * 7127 + barycentrics.y * 4057 + barycentrics.z * 2311))) /
             0xFFFFFFFF,
         float(pcg_hash(frame_index * 2141 + 3 * (barycentrics.x * 3907 + barycentrics.y * 587 + barycentrics.z * 7127))) /
             0xFFFFFFFF);*/
